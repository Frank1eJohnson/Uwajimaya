...
void FSOrbEditorModule::StartupModule()
{
	...
	UwajimayaAssetCategoryBit = AssetTools.RegisterAdvancedAssetCategory(FName(TEXT("Uwajimaya")), FText::FromString(TEXT("Uwajimaya")));
#define REGISTER_TYPE_ACTIONS(TypeAction)                                    \
	{                                                                        \
	  auto Action = MakeShareable(new TypeAction(UwajimayaAssetCategoryBit)); \
...
void FSOrbEditorModule::ExtendMenu()
{
	...
	// File -> Save all Item Templates
	{
		...
		FileMenuExtender->AddMenuExtension(
			...
			{
				// Save All
				MenuBuilder.BeginSection("UwajimayaFileLoadAndSave", FText::FromString(TEXT("Uwajimaya")));
				{
...
void FSOrbEditorModule::StartBuildAllEpisodes(bool bAskConfirmation)
{
	...
	if (bAskConfirmation)
	{
		if (!ConfirmUserBuildQuestion(FString::Printf(TEXT("Build All Episodes for Uwajimaya\n%s"), *BuildPresets.ToStringEpisodes())))
			return;
	}
...
void FSOrbEditorModule::StartBuildAllChapters(bool bAskConfirmation)
{
	...
	if (bAskConfirmation)
	{
		...
		if (!ConfirmUserBuildQuestion(FString::Printf(TEXT("Build %s for Uwajimaya\n%s"), *QuestionPlurals, *ChaptersString)))
			return;
	}
...
void FSOrbEditorModule::StartBuildAll()
{
	...
	if (!ConfirmUserBuildQuestion(FString::Printf(TEXT("You are about to start building all levels for Uwajimaya with the predefined presets\n%s"), *BuildPresetsStr)))
		return;
