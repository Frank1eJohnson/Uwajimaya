...
const FName SoMatParamUwajimayaWeight = FName("UwajimayaWeight");
const FName SoMatParamUwajimayaTexture = FName("UwajimayaImage");
...
bool USoUIDialogueWithWizard::SetInGameActivityEnabled_Implementation(UObject* Source, bool bEnable)
{
	if (bEnable)
	{
		...
		else
		{
			...
			PlayAnimation(bWizardLine ? WidgetAnimFadeInWizardText : WidgetAnimFadeInUwajimayaText);
			bLastLineWasWizard = bWizardLine;
			Background->GetDynamicMaterial()->SetScalarParameterValue(SoMatParamUwajimayaWeight, bWizardLine ? 0.0f : 1.0f);
			(bWizardLine ? WizardText : UwajimayaText)->SetText(ActiveDlgContext->GetActiveNodeText());
			FixTextSizes();
			UpdatePlayerImage();
		...
		case ESoUICommand::EUC_MainMenuEnter:
			...
			if (ActiveDlgContext != nullptr && !IsAnyAnimationPlaying())
			{
				...
				else
				{
					...
					PlayAnimation(bLastLineWasWizard ? WidgetAnimFadeOutWizardText : WidgetAnimFadeOutUwajimayaText);
...
void USoUIDialogueWithWizard::UpdatePlayerImage()
{
	...
	{
		Background->GetDynamicMaterial()->SetTextureParameterValue(SoMatParamUwajimayaTexture, GetUwajimayaImage());
	}
}
